...
export default function EditCar({ open, handleClose, car, onSuccess }: EditCarProps) {
  ...
  const handleNextImage = () => {
    setCurrentImageIndex((prevIndex) => (prevIndex + 1) % (existingImages.length + newImages.length));
  };

  const handlePrevImage = () => {
    setCurrentImageIndex((prevIndex) => (prevIndex - 1 + existingImages.length + newImages.length) % (existingImages.length + newImages.length));
  };
    ...
    >
      ...
      <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, width: '100%' }}>
        ...
        <Stack direction="row" spacing={2} sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Button
            sx={{ flex: 1 }}
            onClick={() => handleRemoveImage(currentImageIndex, currentImageIndex < existingImages.length)}          >
            Remove Image
          </Button>
        </Stack>
        {combinedImages.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '400px', height: '240px', overflow: 'hidden', whiteSpace: 'nowrap' }}>
              <img
                 src={typeof combinedImages[currentImageIndex] === 'string' ? combinedImages[currentImageIndex] as string : URL.createObjectURL(combinedImages[currentImageIndex] as File)}
